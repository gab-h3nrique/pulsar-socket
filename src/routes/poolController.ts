// 200 OK
// 201 Created
// 202 Accepted
// 203 Non-Authoritative Information
// 204 No Content
// 205 Reset Content
// 206 Partial Content

import { PoolModel } from "../models/poolModel";
import Auth from "../socket/auth";
import { PoolType } from "../types/poolType";

const SECRET = String(process.env.SECRET)

// 400 Bad Request
// 401 Unauthorized
// 402 Payment Required
// 403 Forbidden
// 405 Method Not Allowed
// 406 Not Acceptable
// 429 Too Many Requests
// 500 Internal Server Error
// 501 Not Implemented
// 502 Bad Gateway
// 503 Service Unavailable

function factory() {

    
    return {
        
        get: async(req: any, res: any) => {

            const { query } = req;

            const id = query.id ? String(query.id) : null;
            const input = query.input ? String(query.input) : '';

            const startDate = query.startDate ? String(query.startDate) : '';
            const endDate = query.endDate ? String(query.endDate) : '';

            const page = query.page ? Number(query.page) : null;
            const limit = query.limit ? Number(query.limit) : null;

            if(id) return res.status(200).json({ success: true, data: await PoolModel.find(id), message: '' });

            if(!page || !limit) return res.status(200).json({ success: true, data: await PoolModel.get(input), message: '' });

            const index = (Number(page) - 1) * Number(limit)
        
            const { data, total } = await PoolModel.paginated(index, Number(limit), (input || ''), startDate, endDate)
        
            return res.status(200).json({ success: true, data, total, message: '' });
        
        },

        post: async(req: any, res: any) => {

            const { body } = req;

            const item: PoolType = {
                id: body.id ? String(body.id) : undefined,
                name: body.name ? String(body.name) : '',
                token: body.url ? String(body.token) : '',
            }

            if(!item.name) return res.status(400).json({ success: false, data: null, message: 'Name is required.' });
            if(item.id && !item.token) return res.status(400).json({ success: false, data: null, message: 'Token is required.' });

            let data = await PoolModel.upsert(item);

            if(!data.token) data.token = Auth.generateToken(data.id, data.name);

            if(!item.id) data = await PoolModel.upsert(data as any);

            return res.status(200).json({ success: true, data: data, message: '', });
        
        },

        delete: async(req: any, res: any) => {

            const { query } = req;

            const id = query.id ? String(query.id) : undefined;
    
            if(!id) return res.status(400).json({ success: false, data: null, message: 'ID is required' });

            const data = await PoolModel.find(id);

            if(!data) return res.status(404).json({ success: false, data: null, message: 'Data not found' });

            await PoolModel.delete(data.id);
            
            return res.status(200).json({ success: true, data: null, message: 'Data deleted successfully' });
        
        },

    
    }

}

const poolController = factory()

export default poolController
